...
A minimal console app that builds a static site with [PageSharp](../../src/PageSharp).
It depends only on the PageSharp library, so it doubles as a check that the generator
...
PageSharp skips those binaries and still produces a complete HTML site.
