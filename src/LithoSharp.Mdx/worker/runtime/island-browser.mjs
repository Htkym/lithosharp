...
export function registerIsland(descriptor, context) {
  ...
  async function activate() {
    ...
    try {
      const [react, client, {PageContext}, module] = await Promise.all([import('react'), import('react-dom/client'), import('./context.mjs'), descriptor.load()]);
      ...
      state.root = hydrateRoot(element, React.createElement(PageContext.Provider, {value: context}, React.createElement(component, JSON.parse(element.dataset.islandProps))), {
  ...
  } else if (descriptor.strategy === 'idle' && 'requestIdleCallback' in window) {
    const task = requestIdleCallback(activate, {timeout: 2000}); state.cancel = () => cancelIdleCallback(task);
  } else if (descriptor.strategy === 'visible' && 'IntersectionObserver' in window) {
  ...
  } else if (descriptor.strategy === 'media' && 'matchMedia' in window) {
...
if (!globalThis[listenerKey]) {
  ...
  document.addEventListener('lithosharp:before-navigate', () => {
    generation++; for (const state of registry.values()) { state.cancel(); state.root?.unmount(); } registry.clear();
  });
}
