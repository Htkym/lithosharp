...
internal static class SiteQualityValidator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            foreach (var pair in textFiles.OrderBy(pair => pair.Key, StringComparer.Ordinal))
            {
                cancellationToken.ThrowIfCancellationRequested();
                if (!IsHtml(pair.Key, pair.Value)) continue;
                var route = artifactRoutes[pair.Key];
                var document = await parser.ParseDocumentAsync(pair.Value, cancellationToken).ConfigureAwait(false);
                ...
                pages.Add(pair.Key, new ParsedPage(route, document, resolutionBase,
                ...
                links.Add(pair.Key, new HashSet<string>(StringComparer.Ordinal));
            }
            ...
            foreach (var pair in pages)
            {
                ...
                var path = pair.Key;
                var page = pair.Value;
                var canonicalElements = page.Document.QuerySelectorAll("link")
                ...
                {
                    if (string.IsNullOrWhiteSpace(page.Document.Title)) AddMissing("title");
                    foreach (var (attribute, name) in new[] { ("name", "description"), ("property", "og:title"), ("property", "og:description"), ("property", "og:url") })
                        if (!page.Document.QuerySelectorAll("meta").Any(element =>
                ...
                foreach (var element in page.Document.QuerySelectorAll("[href],[src],[srcset],[poster],object[data],form[action],meta[property]"))
                {
                ...
                foreach (var element in page.Document.QuerySelectorAll("style,[style]"))
            ...
            foreach (var pair in textFiles.Where(pair => pair.Key.EndsWith(".css", StringComparison.OrdinalIgnoreCase)))
                CheckCss(pair.Value, new Uri(origin + artifactRoutes[pair.Key].PublicPath), pair.Key, null);

            foreach (var group in pages.Where(pair => !redirects.ContainsKey(pair.Key) && !string.IsNullOrWhiteSpace(pair.Value.Document.Title))
                .GroupBy(pair => Regex.Replace(pair.Value.Document.Title!.Trim(), "\\s+", " "), StringComparer.OrdinalIgnoreCase)
        ...
        {
            foreach (var page in pages.Values) page.Document.Dispose();
        }
    ...
    private sealed record ParsedPage(SiteRoute Route, IDocument Document, Uri ResolutionBase, HashSet<string> Anchors);
}
