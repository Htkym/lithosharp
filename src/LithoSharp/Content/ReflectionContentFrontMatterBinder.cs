    ...
    where TFrontMatter : notnull
{
        ...
        out object? converted)
    {
        ...
        {
            converted = ContentScalarConversion.UnwrapTree(input);
            return true;
        }
    ...
    private static bool TryConvertScalar(object input, Type targetType, out object? converted)
    {
        ...
        {
            return Enum.TryParse(targetType, input as string ?? Convert.ToString(input, CultureInfo.InvariantCulture),
                ignoreCase: false, out converted);
        }
        return ContentScalarConversion.TryConvertScalar(input, targetType, out converted);
    }
